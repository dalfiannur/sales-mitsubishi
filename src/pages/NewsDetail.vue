...
<template>
    <div v-if="data" class="flex justify-center pt-5">
        <div class="flex flex-1 max-w-screen-lg gap-5">
            <div class="flex-1 p-5 bg-white rounded-xl">
                <h2 class="text-3xl font-bold">{{ data.title }}</h2>
                <img class="object-cover w-full mt-5 aspect-video" :src="data.thumbnail" />
                <div class="flex justify-end mt-2">
                    <div class="text-sm">{{ data.createdAt }}</div>
                </div>
                <div class="mt-5 indent-8">{{ data.content }}</div>
            </div>

            <div class="w-52">
                <div class="mb-10 border-b border-white" v-for="item in news" :key="item.slug">
                    <img
                    ...
                    />
                    <div class="py-2">
                        <a href="/berita/2" class="my-2 text-white" @click="$router.push('/berita/2')">{{ item.title }}</a>
                    </div>
                </div>
            </div>
        </div>
